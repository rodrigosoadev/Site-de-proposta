...
interface ContractorInfoStepProps {
  ...
  onChange: (data: ContractorData) => void;
}

export function ContractorInfoStep({ value, onChange }: ContractorInfoStepProps) {
  const handleChange = (field: keyof ContractorData, fieldValue: string | boolean) => {
    onChange({
      ...value,
      [field]: fieldValue
    });
  ...
  return (
    <div className="space-y-4">
      <p className="text-sm text-muted-foreground mb-4">
        Preencha os dados do contratante (quem está contratando o serviço).
      </p>
      
      <div className="flex items-center space-x-2">
        <Checkbox 
          id="is-company-contractor"
          checked={value.isCompany}
          onCheckedChange={(checked) => handleChange('isCompany', checked === true)}
        />
        <Label htmlFor="is-company-contractor" className="text-sm">
      ...
      <div>
        ...
        <Input
          ...
          placeholder={value.isCompany ? "Empresa S.A." : "Nome completo do contratante"}
          required
        />
      </div>

      <div>
        <Label htmlFor="contractor-document">
      ...
      {value.isCompany && (
        <div>
          <Label htmlFor="contractor-representative">
